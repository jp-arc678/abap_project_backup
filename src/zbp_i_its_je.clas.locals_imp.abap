...
*--------------------------------------------------------------------*
* GLOBAL AUTHORIZATION - this is what hides the "Create" button on the
* list report for everyone who is not accounting
*--------------------------------------------------------------------*
  METHOD get_global_authorizations.

    IF requested_authorizations-%create = if_abap_behv=>mk-on.
      result-%create = COND #( WHEN is_accounting( ) = abap_true
...
*--------------------------------------------------------------------*
  METHOD get_instance_features.
    ...
    result = VALUE #( FOR entry IN entries
      LET is_frozen = COND abap_bool( WHEN entry-PostingStatus = 'P' OR is_acc = abap_false
                                      THEN abap_true ELSE abap_false )
      IN
      ( %tky = entry-%tky
  ...
  METHOD getItemFeatures.
    ...
    LOOP AT keys INTO DATA(item_key).
      ...
      DATA(is_frozen) = COND abap_bool(
                          WHEN is_acc = abap_false
                            OR ( sy-subrc = 0 AND header-PostingStatus = 'P' )
                          THEN abap_true ELSE abap_false ).
